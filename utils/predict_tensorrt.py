...
class Inference_TensorRT:
    def __init__(self, onnx_file_path,
        ...
        temp_trt_file = os.path.join(cfg.trt_file_cache, os.path.basename(onnx_file_path).replace('.onnx', '.trt'))
        ...
        if load_trt_flag:
            ...
            # serialize engine
            if not os.path.exists(cfg.trt_file_cache):
                os.makedirs(cfg.trt_file_cache)
            logging.info('Serialize the engine for fast init.')
            with open(os.path.join(cfg.trt_file_cache, os.path.basename(onnx_file_path).replace('.onnx', '.trt')), 'wb') as fout:
